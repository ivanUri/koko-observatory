...
import { AlertTriangle, Bookmark, CheckCircle2, CircleDot, Code2, GitBranch, LoaderCircle, PlayCircle, RotateCcw, Search, ShieldCheck, SlidersHorizontal, SquareArrowOutUpRight, XCircle } from "lucide-react";
...
export function ExecutionPanel() {
  ...
  const [validationPhase, setValidationPhase] = useState<"idle" | "validating" | "success">("idle");
  ...
  const badgeState = !source
    ? "none"
    : selectedCheckpoint?.replayable
      ? "replayable"
      : sourceCheckpoints.length
        ? "captured"
        : "pending";
  ...
  useEffect(() => () => validationTimers.current.forEach((timer) => window.clearTimeout(timer)), []);
  ...
  const validatePlan = () => {
    ...
    const result = `${inputs.length} captured inputs · ${disabledKeys.length} disabled · ${Object.keys(overrides).length} edited · ${breakpointKeys.length} breakpoint${breakpointKeys.length === 1 ? "" : "s"}. ${mode === "strict" ? "No uncaptured network request will be allowed." : "Fallback mode can contact the network."}`;
    validationTimers.current.push(window.setTimeout(() => { setValidationPhase("success"); setPlanMessage(result); }, 750));
    validationTimers.current.push(window.setTimeout(() => setValidationPhase("idle"), 4_000));
  };
  ...
  return <main className="workspace replay-workspace">
